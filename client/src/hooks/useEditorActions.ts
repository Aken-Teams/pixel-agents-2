...
export function useEditorActions(
  ...
  // Called by useServerMessages on layoutLoaded to set the initial checkpoint
  ...
  const saveLayout = useCallback((layout: OfficeLayout) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      wsClient.postMessage({ type: 'saveLayout', layout })
  ...
  const handleOpenClaude = useCallback(() => {
    wsClient.postMessage({ type: 'openClaude' })
  }, [])
