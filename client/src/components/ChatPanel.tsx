        ...
        <div className="team-tabs" style={{
          ...
          {orchestratorMember && (
            ...
            >
              ...
              {!orchestratorBusy && dispatchedTasks.some((t) => t.completed) && (
                <span style={{
                  width: 6,
                  height: 6,
                  borderRadius: '50%',
...
function stripInternalBlocks(text: string): { cleanText: string; tasks: { skillId: string; description: string }[] } {
  ...
  let cleaned = text
    ...
    .replace(/\[TASK:\w[\w-]*\]/g, '')
    .replace(/\[\/TASK\]/g, '')
    // Remove SCHEDULE blocks (full and partial during streaming)
    .replace(/\[SCHEDULE\]\s*[\s\S]*?\s*\[\/SCHEDULE\]/g, '')
    .replace(/\[SCHEDULE\][\s\S]*$/g, '')  // partial block during streaming
...
function MessageBubble({ message, assistantName, isStreaming, teamMembers, dispatchedTasks, isSubAgentTab, orchestratorName }: {
  ...
  // Skip rendering pure result feedback messages (internal orchestrator feedback)
  const isResultFeedback = !isUser && message.content.startsWith('[RESULT:')
  if (isResultFeedback) return null
